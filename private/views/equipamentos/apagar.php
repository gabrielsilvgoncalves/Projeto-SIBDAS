<?php
require_once __DIR__ . '/../../includes/funcoes.php';
redirect_if_not_logged();
?>
<?php include '../../includes/header.php'; ?>
<?php include '../../includes/nav.php'; ?>

<div class="container-fluid">
    <div class="row">
        <?php include '../../includes/sidebar.php'; ?>

        <main class="col-md-9 col-lg-10 p-4 d-flex align-items-start justify-content-center" style="background-color: #f2f2f2;">
            <div class="caixa-apagar">
                <div class="icone-alerta">
                    <i class="fas fa-triangle-exclamation"></i>
                </div>
                <h4 class="fw-bold mb-2">Confirmar Remoção</h4>
                <p class="text-muted">Tem a certeza que pretende remover o seguinte equipamento?</p>
                <p class="text-muted small">Esta ação é irreversível. Todos os dados associados ao equipamento serão eliminados.</p>

                <div class="info-item">
                    <strong>Código:</strong> 04.002.00<br>
                    <strong>Designação:</strong> Monitor multiparamétrico<br>
                    <strong>Marca / Modelo:</strong> Philips / IntelliVue MP5<br>
                    <strong>Nº Série:</strong> MP5-2022-45873<br>
                    <strong>Serviço:</strong> Unidade de Cuidados Intensivos
                </div>

                <div class="alert alert-danger py-2 px-3 mt-3 small text-start">
                    <i class="fas fa-exclamation-circle me-1"></i>
                    <strong>Atenção:</strong> Este equipamento tem documentos e registos de garantia associados que também serão removidos.
                </div>

                <div class="botoes mt-4">
                    <a href="lista.php" class="botao-cancelar">
                        <i class="fas fa-xmark me-1"></i> Cancelar
                    </a>
                    <a href="lista.php" class="botao-confirmar" onclick="confirmarRemocao(event)">
                        <i class="fas fa-trash-can me-1"></i> Confirmar Remoção
                    </a>
                </div>
            </div>
        </main>
    </div>
</div>

<script>
    function confirmarRemocao(e) {
        e.preventDefault();
        if (confirm('Esta ação é irreversível. Confirma a remoção do equipamento?')) {
            alert('Equipamento removido com sucesso.');
            window.location.href = 'lista.php';
        }
    }
</script>

<?php include '../../includes/footer.php'; ?>
