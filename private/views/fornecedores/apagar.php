...
try {
    ...
    $stmt = $ligacao->prepare("SELECT nome, nif FROM fornecedores WHERE id = :id AND deleted_at IS NULL LIMIT 1");
    $stmt->execute([':id' => $id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['confirmar'] ?? '') === '1') {
    try {
        $stmt = $ligacao->prepare("UPDATE fornecedores SET deleted_at = NOW() WHERE id = :id");
        $stmt->execute([':id' => $id]);
        $ligacao = null;
    ...
    <div class="row">
        ...
        <main class="col-md-9 col-lg-10 p-4 d-flex align-items-start justify-content-center" style="background-color: #f2f2f2;">
            <div class="caixa-apagar">
                ...
                <div class="info-item">
                    <strong>Empresa:</strong> <?= htmlspecialchars($f->nome) ?><br>
                    <strong>NIF:</strong> <?= htmlspecialchars($f->nif ?? '—') ?>
                </div>
